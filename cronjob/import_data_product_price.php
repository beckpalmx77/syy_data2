...
$sql_find = "SELECT COUNT(*) FROM ims_product WHERE product_id = :product_id AND product_key = :product_key AND price_code = :price_code";
$stmt_find = $conn->prepare($sql_find);

$sql_update = "UPDATE ims_product SET name_t=:name_t , brand_id=:brand_id , pgroup_id=:pgroup_id , price=:price 
               WHERE product_id = :product_id AND product_key = :product_key AND price_code = :price_code";
$stmt_update = $conn->prepare($sql_update);

$sql_insert = "INSERT INTO ims_product(product_key,product_id,pgroup_id,name_t,brand_id,price_code,price) 
...
try {
    ...
    $conn->commit();
    echo "Import product price completed. Updated: $update_count, Inserted: $insert_count\n\r";
} catch (Exception $e) {
    $conn->rollBack();
