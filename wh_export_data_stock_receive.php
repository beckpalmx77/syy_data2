...
// ==========================================================================================
// *** 2. Define Screen ID ***
$current_screen_id = "wh_stock_receive";
// ==========================================================================================
...
// --- 3.1 กำหนดรายการ ICCAT_CODE ---
$iccat_list = [
    '001-AP01', '002-BKT01', '003-BS05', '004-BS06', '005-O01',
    '006-RS01', '007-T%01', '008-TR01', '009-CO01', '010-T%01',
    '011-LO01', '012-T%01', '013-AP01', '014-CT01', '015-FK01',
    '016-GY01', '017-HK01', '018-KENDA', '019-MS01', '020-MX01',
    '023-NT01', '024-O01', '025-T%01', '026-T01', '027-TOYO01',
    '028-YK01', '101-AT01', '101-BS01', '101-BS02', '101-BS03',
    '101-BS04', '101-DS01', '101-DS02', '101-DS03', '101-FS01',
    '101-FS02', '101-FS03', '101-FS04', '101-LLIT01', '101-ML01',
    '101-SIM01', '101-VR01', '102-CAV01', '102-CBG01', '102-CBS01',
    '102-CDS01', '102-CKL01', '102-CKS01', '102-CML01', '102-CMT01',
    '102-VR01', '103-LBS01', '103-LDS01', '103-LKS01', '103-LML01',
    '103-LMT01', '103-LVR01', '201-AT01', '201-BF01', '201-BS01',
    '201-DL01', '201-DS01', '201-DT01', '201-FS01', '201-FT01',
    '201-GT01', '201-LEAO01', '201-LLIT01', '201-ML01', '201-PX01',
    '201-SIM01', '201-TBB01', '201-VR01', '202-NK01'
];


// แปลง Array เป็น String สำหรับ SQL
$in_clause = "'" . implode("', '", $iccat_list) . "'";

// --- 3.2 Fetch ICCAT ---
$sql_iccat = "SELECT DISTINCT ICCAT_CODE, ICCAT_NAME 
              FROM ICCAT 
              WHERE ICCAT.ICCAT_CODE IN ($in_clause) 
              ORDER BY ICCAT_CODE ASC";

$stmt1 = $conn_sqlsvr->prepare($sql_iccat);
$stmt1->execute();
$iccats = $stmt1->fetchAll(PDO::FETCH_ASSOC);

// --- 3.3 Fetch BRAND (ถ้าต้องการใช้ให้เอา Comment ออก) ---
/*
                    ...
                    <div class="col-lg-12">
                        <div class="card mb-4">
                            <div class="card-body">
                                        ...
                                        <div class="row align-items-end">
                                            ...
                                            <div class="col-md-8 mb-2">
                                                <div class="d-flex justify-content-end flex-wrap">
                                                    ...
                                                    <button type="button" class="btn btn-secondary mr-1 mb-1" id="btn_clear_all" style="white-space: nowrap;">
                                                        <i class="fas fa-eraser"></i> ล้างค่า
                                                    </button>

                                                    <button type="submit" form="exportForm" class="btn btn-primary mr-1 mb-1"
                                                            formaction="export_process/wh_print_stock_all_pdf"
                                                            formtarget="_blank" style="white-space: nowrap;">
                                                        <i class="fa fa-file-pdf"></i> รายการรับเข้า PDF
                                                    </button>

                                                    <button type="submit" form="exportForm" class="btn btn-primary mr-1 mb-1"
                                                            formaction="export_process/wh_export_data_stock_balance_all_pdf"
                                                            formtarget="_blank" style="white-space: nowrap;">
                                                        <i class="fa fa-file-pdf"></i> ยอดคงเหลือ PDF
                                                    </button>

                                                    <button type="submit" form="exportForm" class="btn btn-success mb-1" style="white-space: nowrap;">
                                                        <i class="fa fa-file-excel"></i> ยอดคงเหลือ Excel
                                                    </button>
                                ...
                                <form method="post" id="exportForm" action="export_process/wh_export_data_stock_balance_all_process.php">

                                    <fieldset class="mb-4">
                                        <legend class="text-primary">
                                            <i class="fas fa-calendar-alt mr-2"></i> ช่วงเวลา (Date Range)
                                        </legend>
                                        <div class="row">
                                            <div class="col-md-3">
                                                <label class="font-weight-bold text-primary">วันที่เริ่มต้น (Start):</label>
                                                <input type="date" class="form-control" name="start_date" id="start_date" value="<?php echo $current_date; ?>" required>
                                            </div>
                                            <div class="col-md-3">
                                                <label class="font-weight-bold text-primary">วันที่สิ้นสุด (End):</label>
                                                <input type="date" class="form-control" name="end_date" id="end_date" value="<?php echo $current_date; ?>" required>
                                            </div>
                                        </div>
                                    </fieldset>
                                    ...
                                    <fieldset class="mb-4">
                                        <legend class="text-info">
                                            <input type="checkbox" class="select-all" id="check_all_icc" checked>
                                            ...
                                            <?php foreach ($iccats as $row): ?>
                                                <div class="col-md-4 col-sm-6">
                                                    <div class="custom-control custom-checkbox">
                                                        <label>
                                                            <input type="checkbox" class="chk-icc" name="icc_codes[]" value="<?= htmlspecialchars($row['ICCAT_CODE']) ?>" checked>
...
<script>
    $(document).ready(function() {
        ...
        function loadPreset(preset_id) {
            ...
            if(preset_id == "") {
                // Reset Checkbox ทั้งหมดเป็น False ก่อน (หรือจะให้เป็น True ตาม default ก็ได้ แต่ปกติปุ่ม Clear/Reset มักจะเคลียร์ออก)
                // ถ้าอยากให้ Reset แล้วกลับมาเลือกทั้งหมด ให้แก้ false เป็น true ด้านล่าง
